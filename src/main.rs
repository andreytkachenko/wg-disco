use std::{collections::HashMap, fs, net::SocketAddrV4, pin::pin};
...
use wg::{Key, Peer, WireguardApi, cmd::WgCmdBackend, config::WgConfig, peer::WgPeerInfo};
...
async fn main() -> Result<(), Error> {
    ...
    // let discover = discover::fake::FakeDiscover::default();
    let (endpoint, local_port) = discover.discover().await.unwrap();

    if config.interface.listen_port.is_none() {
        wg.set_listen_port(&iface, local_port)?;
    };

    let peer = Peer(key, endpoint);
    ...
    let mut signaling = IrcSignaling::connect(cfg, key, config.peers.iter().map(|x| &x.public_key))
        .await
        .unwrap();
